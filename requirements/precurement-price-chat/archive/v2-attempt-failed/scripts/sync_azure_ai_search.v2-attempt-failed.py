...
INDEX_SCHEMA = {
    ...
    "fields": [
        ...
        {"name": "qty", "type": "Edm.String", "retrievable": True},
        {"name": "qty_min", "type": "Edm.Int32", "filterable": True, "sortable": True, "retrievable": True},
        {"name": "qty_max", "type": "Edm.Int32", "filterable": True, "sortable": True, "retrievable": True},
        {"name": "notes", "type": "Edm.String", "searchable": True, "retrievable": True},
        {"name": "product_description", "type": "Edm.String", "searchable": True, "retrievable": True, "analyzer": "th.microsoft"},
        {"name": "related_products", "type": "Collection(Edm.String)", "retrievable": True},
...
def main() -> None:
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            existing_fields = {field["name"] for field in existing_schema.get("fields", [])}
            required_fields = {
                field["name"]: field
            ...
            if required_fields:
                existing_schema["fields"].extend(required_fields.values())
                response = requests.put(
                    f"{index_url}&allowIndexDowntime=true",
                    headers=search_headers,
                    json=existing_schema,
                    timeout=90,
                )
                response.raise_for_status()
                print("Updated index schema: " + ", ".join(sorted(required_fields)))
    ...
    response = requests.post(
        query_url,
        headers=search_headers,
        json={"search": "ร่มโค้ก", "select": "product_name,variant_description,price,vendor_name", "top": 3},
        timeout=30,
    )
    response.raise_for_status()
