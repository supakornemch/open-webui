#!/usr/bin/env python3
"""Deploy the Fabric Open WebUI tool without shell JSON quoting.

Required environment variables for a live deploy:
    OWUI_URL       Open WebUI base URL, for example https://genie.haadthip.com
    OWUI_TOKEN     Open WebUI API bearer token

Usage:
    python3 scripts/deploy/deploy-fabric-tool.py --dry-run
    python3 scripts/deploy/deploy-fabric-tool.py
"""
...
def build_body() -> dict[str, Any]:
    ...
    # Open WebUI derives this manifest from the tool's opening docstring.
    header = re.search(r'^"""\n(?P<header>.*?)\n"""', content, re.DOTALL)
    if not header:
    ...
    for line in header.group("header").splitlines():
        if ":" in line:
            ...
            if key in {"title", "author", "version", "required_open_webui_version"}:
    ...
    return {
        ...
        "content": content,
        "meta": {
            "description": "ค้นหาและวิเคราะห์ข้อมูลจาก Microsoft Fabric Data Warehouse",
            "manifest": manifest,
        },
        "access_control": None,
    }
...
def main() -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="validate and write payload locally; do not call OWUI")
    parser.add_argument("--payload", type=Path, help="optional path for the generated JSON payload")
    ...
    base_url = env("OWUI_URL").rstrip("/")
    token = env("OWUI_TOKEN")
    headers = {
