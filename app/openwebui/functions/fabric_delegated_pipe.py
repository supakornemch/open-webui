"""
title: Genie AI — Fabric Delegated Query (Pipe)
author: Haadthip DIO
version: 1.0.0
...
FABRIC_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            "customer info, credit/AR, targets, or any quantitative business question."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "sql_query": {
                    "type": "string",
                    "description": (
                        "T-SQL SELECT query. Must use explicit column names (no SELECT *), "
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        MAX_TOOL_ROUNDS: int = Field(default=6, description="Max tool loop iterations")
    ...
    def pipe(self, body: dict, __user__: dict = None, __oauth_token__: dict = None, __event_emitter__=None) -> str:
        ...
        for _ in range(max(1, min(self.valves.MAX_TOOL_ROUNDS, 8))):
